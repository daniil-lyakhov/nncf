...
class NNCFProfiler:
    ...
    def collect_activations(
        ...
        target_names = set([key for key in node_keys if regexp.search(key)])
    ...
    def calculate_stats(self, data: ActivationData, statistics: Optional[list[str]] = None) -> pd.DataFrame:
        ...
        for stat in statistics:
            if stat not in self.STATISTICS:
                raise ValueError(f"Unknown statistic '{stat}'. Available: {list(self.STATISTICS.keys())}")
    ...
    def compare_activations(
        ...
        for metric in metrics:
            if metric not in self.COMPARATORS:
                raise ValueError(f"Unknown comparator '{metric}'. Available: {list(self.COMPARATORS.keys())}")
    ...
    def plot(self, plot_type: str, *args, **kwargs):
        ...
        if plot_type not in self.VISUALIZERS:
            raise ValueError(f"Unknown plot type '{plot_type}'. Available: {list(self.VISUALIZERS.keys())}")
