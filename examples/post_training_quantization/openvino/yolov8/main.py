...
def main_export_not_strict():
    ...
    # ex_model = torch.compile(ex_model, backend="openvino")

    # exported_model = torch.export.export(ex_model, args=(batch["img"],))
    # ov_model = ov.convert_model(exported_model, example_input=batch["img"], input=batch["img"].shape)
