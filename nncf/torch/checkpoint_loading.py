...
def load_state(model: torch.nn.Module, state_dict_to_load: dict, is_resume: bool = False,
    ...
    model_bn = [x for x in list(model_state_dict.keys()) if 'batch' in x.lower()]
    ckpt_bn = [x for x in list(state_dict_to_load.keys()) if 'batchnorm2d' in x.lower()]
    ...
    for bn_ckpt_name in ckpt_bn:
        weight = state_dict_to_load.pop(bn_ckpt_name)
        new_bn_name = bn_ckpt_name.replace('BatchNorm2d', 'NNCFBatchNorm')
        new_bn_name = '.'.join(new_bn_name.split('.')[1:])
        assert new_bn_name in model_state_dict
        state_dict_to_load[new_bn_name] = weight
