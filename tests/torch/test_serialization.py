...
@pytest.mark.parametrize("priority", TwoConvTestModel.PRIORITIES)
def test_serialize_load_command(target_type, command_builder, priority):
    dummy_op_state = "DUMMY_OP_STATE"
    command = TwoConvTestModel.create_one_command(command_builder, target_type, priority, dummy_op_state)
